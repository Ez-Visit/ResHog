...
### Q: WAL 文件（data.db-wal）膨胀到几个 G 怎么办？

正常情况下 WAL 文件应稳定在 100MB 以下。如果异常膨胀（数 G 以上），通常是 checkpoint 失效导致的。处理方法：

```powershell
# 1. 停止服务（避免数据竞争）
Stop-Service ResHog

# 2. 手动触发 TRUNCATE checkpoint（回放 WAL 并截断文件）
...
新版本（2026-07-21 之后）已在服务启动时自动 TRUNCATE WAL，并在运行时每 10 分钟做一次 PASSIVE checkpoint，通常无需手动操作。如果升级后首次启动慢（数分钟），是在回放历史 WAL，属正常现象。
